  ...
  curr.fields.forEach(cf => {
    ...
    if (oldIsSerial && !currIsSerial) {
      ...
      alterList.push(
        ...
        `ALTER TABLE ${quoteId(curr.name)} ALTER COLUMN ${quoteId(cf.column_name)} DROP IDENTITY ;`
      )
    } else if (!oldIsSerial && currIsSerial) {
            /* ✅ 新增自增：integer -> identity，PG 10+ 写法 */
      const tmpName = `${cf.column_name}_new_${Date.now()}`;
      alterList.push(
        // 1. 新建普通 int 列
        // `ALTER TABLE ${quoteId(curr.name)} ADD COLUMN ${quoteId(tmpName)} int NOT NULL ;`,
        // // 2. 搬运数据
        ...
        // 5. 原地改成 identity（BY DEFAULT 允许以后手动插入）
        // `ALTER TABLE ${quoteId(curr.name)} ALTER COLUMN ${quoteId(cf.column_name)} 
        // ADD GENERATED BY DEFAULT AS IDENTITY (START WITH 1);`

        `ALTER TABLE ${quoteId(curr.name)} ALTER COLUMN ${quoteId(cf.column_name)} 
        ADD GENERATED BY DEFAULT AS IDENTITY ;`
        // ,
        // // 6. 把序列起点拉到当前最大值，避免重复
        // `SELECT setval(pg_get_serial_sequence('${curr.name}', '${cf.column_name}'), 
        //               coalesce(max(${quoteId(cf.column_name)}), 1), true) FROM ${quoteId(curr.name)};`
      );
    }
